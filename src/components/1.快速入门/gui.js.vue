...
selFolder.close();
// 下拉菜单
selFolder.add(mesh.position, 'y', [-25, 0, 50, 100]).name('球形y-下拉选择').onChange(function(value){
    mesh.position.y = value;
});
selFolder.add(mesh.position, 'x', {
    left: -100,
    center: 0,
    right: 100
}).name('球形位置选择').onChange(function (value) {
...
// 材质颜色color
matFolder.addColor(obj, 'color').name('物体颜色').onChange(function(value){
    material.color.set(value);
});
// 材质高光颜色specular
matFolder.addColor(obj, 'specular').name('物体高光颜色').onChange(function(value){
